# Wacom Art Pen 1 (KP-701E) notes

## Overview

One of the best pens Wacom ever created and one of the few that supports Barrel Rotation.

Wacom discontinued this pen sometime around the end of 2023 or beginning of 2024. It is succeeded by the [Wacom Art Pen 2 (ACP-700)](wacom-acp700-notes.md).

See: [Wacom Art Pen 2 (ACP-700) vs Art Pen 1 (KP-701E)](wacom-acp500-vs-kp701e.md)

## Naming

This pen is often called the "Intuos 4 Art Pen".&#x20;

## Eraser

This pen does have an eraser.

## Buttons

This model has 2 buttons in a rocker configuration.

## IAF

| Statistic | IAF (gf) |
| --------- | -------- |
| Min       | 1.5      |
| Median    | 1.5      |
| Max       | 1.5      |



| Pen           | Inventory ID | IAF (gf) | Source   |
| ------------- | ------------ | -------- | -------- |
| Wacom Art Pen | WAP.0025     | 1.5      | measured |
| Wacom Art Pen | WAP.0026     | 1.5      | measured |

## Maximum physical pressure

| Statistic | Pmax (gf) |
| --------- | --------- |
| Min       | 279.4     |
| Median    | 354.3     |
| Max       | 481.6     |

| Inventory ID | Driver | Highest measured (gf) | Pmax estimate (gf) |
| ------------ | ------ | --------------------- | ------------------ |
| WAP.0025     | WACOM  | 486.9                 | 481.6              |
| WAP.0026     | WACOM  | 281.0                 | 279.4              |
| WAP.0025     | WACOM  | 432.2                 | 425.8              |
| WAP.0026     | WACOM  | 311.0                 | 282.9              |

## Photos

<figure><img src="../../../.gitbook/assets/wacom-kp701e-notes-1.jpg" alt=""><figcaption></figcaption></figure>

<figure><img src="../../../.gitbook/assets/wacom-kp701e-notes-2.jpg" alt=""><figcaption></figcaption></figure>

<figure><img src="../../../.gitbook/assets/wacom-kp701e-notes-3.jpg" alt=""><figcaption></figcaption></figure>

<figure><img src="../../../.gitbook/assets/wacom-kp701e-notes-4.jpg" alt=""><figcaption></figcaption></figure>
