# Recommended apps

## Overview

Keep in mind that I use my drawing tablets in very specific ways. And I certainly don't use every kind of application. But if you are curious below you can find the apps that I recommend and why I recommend them.

If you want to see a full list of applications go here: [Apps](./).

## Recommendations

* [Clip Studio Paint](../catalog/apps/clip-studio-paint/)
  * (Windows, MacOS, Android, iOS)
* [Krita](../catalog/apps/krita.md)
  * (Windows, Linux, MacOS)
* [Affinity](../catalog/apps/affinity.md)
  * (Windows, MacOS)
* **Affinity Design** - no notes
  * (Windows, MacOS)
* [Adobe Photoshop](../catalog/apps/adobe-photoshop.md)
  * (Windows, MacOS)
* [Procreate](../catalog/apps/procreate.md)
  * (iOS)
* **Rebelle** - no notes
  * (Windows, MacOS)
* [Infinite Painter](../catalog/apps/infinite-painter.md) - no notes
  * (Android, iOS)
* [Concepts app](../catalog/apps/concepts-app.md) - no notes
  * (Windows, Android, iOS, ChromeOS)
