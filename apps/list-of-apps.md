# List of apps

## Creative 2D

* **Adobe Fresco** (Windows, iOS, MacOS) \[[**links**](../catalog/catalog-apps/fresco.md) ] FREE
* **Affinity (Windows, MacOS) \[**[**links**](../catalog/catalog-apps/affinity/)**,** [**notes**](../catalog/catalog-apps/affinity/affinity-notes.md)**] FREE**
* **ArtFlow Studio** (Android, ChromeOS) \[[**links**](http://artflowstudio.com/)]
* **ArtRage** (Windows, macOS, iOS, Android) \[[**links**](../catalog/catalog-apps/artrage.md)]
* **Clip Studio Paint** (Windows, MacOS, Android, iOS) \[[**links**](../catalog/catalog-apps/clip-studio-paint/)]
* **Drawpile** (Web, Windows, MacOS, Android, Linus) \[[https://drawpile.net/](https://drawpile.net/)]
* **Expresii** (Windows) \[[**links**](../catalog/catalog-apps/expresii.md)]
* **Firealpaca** (Windows, MacOS) \[[**links**](../catalog/catalog-apps/fireaplaca.md)]
* **HEAVYPAINT** (iOS, Android, Web) \[[**links**](../catalog/catalog-apps/heavypaint.md)]
* **HiPaint** (Android, iOS) \[[**links**](../catalog/catalog-apps/hipaint.md)]
* **Ibis Paint** (Windows, iOS, Android) \[[**links**](../catalog/catalog-apps/ibis-paint.md) ]
* **Infinite Design** (Android) \[[https://www.infinitestudio.art/](https://www.infinitestudio.art/) ]
* **Infinite Painter** (Android, iOS) \[[**links**](list-of-apps.md#infinite-painter) ]
* **Kleki** (web) \[[**links**](../catalog/catalog-apps/kleki.md) ] <mark style="background-color:purple;">**FREE**</mark> &#x20;
* **Kreska.art** (web) \[[**links**](https://kreska.art/)] **FREE**
* **Krita** (Windows, Linux, MacOS) \[[**links**](../catalog/catalog-apps/krita/)] <mark style="background-color:purple;">**FREE**</mark>
* **Leonardo** (Windows) \[[**links**](../catalog/catalog-apps/leonardo.md)]
* **Magma** (web) \[[https://magma.com/](https://magma.com/)] FREE
* **Medibang Paint** (Windows, MacOS) \[[**links**](../catalog/catalog-apps/medibang.md)]
* **Medibang Pro** (iOS) \[[**links**](../catalog/catalog-apps/medibang-pro.md)]
* **Microsoft Paint** (aka MSPAINT) (Windows) \[[**links**](../catalog/catalog-apps/microsoft-paint.md)]
* **MyPaint** (Windows, Linux) \[[**links**](../catalog/catalog-apps/mypaint.md)]
* **Paint Tool Sai** (Windows) \[[**links**](../catalog/catalog-apps/painttool-sai.md)]
* **Painter** (Windows, MacOS) \[[**links**](../catalog/catalog-apps/painter.md)]
* **Paintstorm Studio** (Windows, macOS, iOS, Linux) \[[**links**](../catalog/catalog-apps/paintstorm-studio.md)]
* **Procreate** (iOS) \[[**links**](../catalog/catalog-apps/procreate/)]
* **Rebelle** (Windows, MacOS) \[[**links**](../catalog/catalog-apps/rebelle.md)]
* **StylusX** (Windows) \[[links](../catalog/catalog-apps/stylusx.md)]

## Sprites and pixel art

* **DPaint-js** ([https://www.stef.be/dpaint/](https://www.stef.be/dpaint/)) <mark style="background-color:purple;">**FREE**</mark>
* **Piskel** (Web) \[[https://piskelapp.com](https://piskelapp.com/)] <mark style="background-color:purple;">**FREE**</mark>
* **PixiEditor** (Windows, MacOS, Linux) \[[**links**](../catalog/catalog-apps/pixieditor.md)]
* **pixilart** (web) \[[https://www.pixilart.com/](https://www.pixilart.com/)]

## Sketching

* **Concepts** (Windows, Android, iOS, ChromeOS) \[[**links**](../catalog/catalog-apps/concepts-app.md)]
* **Sketchbook** (iOS, Android) \[[https://www.sketchbook.com/](https://www.sketchbook.com/)]
* **Sketchbook Pro** (MacOS, Windows) \[[https://www.sketchbook.com/](https://www.sketchbook.com/)]

## Vector

* **Adobe Illustrator** (Windows, MacOS) \[]
* **Inkscape** (Windows, MacOS, Linux) \[[**links**](../catalog/catalog-apps/inkscape.md)] <mark style="background-color:purple;">**FREE**</mark>
* **VectorInk** (Online, Android)  \[[Google app link](https://play.google.com/store/apps/details?id=com.app.vectorink)]

## 3D&#x20;

* **Autodesk Maya**  \[[https://www.autodesk.com/products/maya/](https://www.autodesk.com/products/maya/)]
* **Blender** (Linux, MacOS, Windows, BSD)  \[[**links**](../catalog/catalog-apps/blender.md)] <mark style="background-color:purple;">**FREE**</mark>
* **Feather 3D** (iPad) \[[**links**](../catalog/catalog-apps/feather-3d.md)]
* **nomadsculpt** (iOS, Android) \[ [https://nomadsculpt.com/](https://nomadsculpt.com/)]
* **Spline** (web) \[ - [https://spline.design/](https://spline.design/) ]
* **Voxelcanvas** (web) \[[https://voxelcanvas.io/](https://voxelcanvas.io/)]
* **ZBrush** (Windows, MacOS, iPadOS) \[[https://www.maxon.net/en/zbrush](https://www.maxon.net/en/zbrush)]

## Photo editing

* **Adobe Photoshop** (Windows, MacOS) \[[**links**](../catalog/catalog-apps/photoshop.md)]
* **Photopea** (web) \[[https://www.photopea.com/](https://www.photopea.com/) ]

## Presenting

* **ZoomIt** (Windows) \[[**links**](../catalog/catalog-apps/zoomit.md)] **FREE**

## **Diagramming and whiteboarding**

* **Visio** - [https://www.microsoft.com/en-us/microsoft-365/visio](https://www.microsoft.com/en-us/microsoft-365/visio)&#x20;
* **Excalidraw** - [https://excalidraw.com/](https://excalidraw.com/)&#x20;
* **Draw.io** - [diagrams.net](https://www.drawio.com/)
* **Microsoft whiteboard** (windows, iOS, web) - [https://www.microsoft.com/en-us/microsoft-365/microsoft-whiteboard/digital-whiteboard-app](https://www.microsoft.com/en-us/microsoft-365/microsoft-whiteboard/digital-whiteboard-app)
* **Miro** - [https://miro.com/](https://miro.com/)&#x20;
* **Lovable** - [https://lovable.dev/](https://lovable.dev/) &#x20;

## Utilities

* Tablet Pro Studio (Windows) \[[https://tabletpro.com/](https://tabletpro.com/)]
