...
* **One cable.** A single USB-C cable carries power, data, and video between your computer and the tablet. This is the simplest and most elegant setup — but it is also the hardest to get working, because all three requirements have to be met in three separate places: the tablet's USB-C port, the computer's USB-C port, AND the cable. The requirement sections below (DP alt mode, power, ports, and cables) describe what the one-cable connection needs.
